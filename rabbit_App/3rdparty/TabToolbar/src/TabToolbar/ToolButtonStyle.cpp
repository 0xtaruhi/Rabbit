...
void TTToolButtonStyle::drawControl(ControlElement element,
                                    const QStyleOption *opt, QPainter *p,
                                    const QWidget *widget) const {
  if (element == CE_ToolButtonLabel) {
    if (const QStyleOptionToolButton *toolbutton =
            static_cast<const QStyleOptionToolButton *>(opt)) {
      ...
      if (((!hasArrow && toolbutton->icon.isNull()) &&
        ...
        if (!toolbutton->icon.isNull()) {
          ...
          pm = toolbutton->icon.pixmap(
              toolbutton->rect.size().boundedTo(toolbutton->iconSize), mode,
              state);
          pmSize = pm.size();
        }
        ...
        if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly) {
          ...
          if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon) {
            ...
            if (!hasArrow)
              proxy()->drawItemPixmap(p, pr, Qt::AlignTop | Qt::AlignHCenter,
                                      pm);
            ...
            if (!hasArrow)
              proxy()->drawItemPixmap(
                  p, QStyle::visualRect(opt->direction, rect, pr),
                  Qt::AlignTop | Qt::AlignHCenter, pm);
            alignment |= Qt::AlignTop | Qt::AlignHCenter;
          }
          tr.translate(shiftX, shiftY);
          proxy()->drawItemText(p, QStyle::visualRect(opt->direction, rect, tr),
          ...
          rect.translate(shiftX, shiftY);
          if (!hasArrow)
            proxy()->drawItemPixmap(p, rect, Qt::AlignTop | Qt::AlignHCenter,
                                    pm);
        }
      }
    }
...
void TTToolButtonStyle::drawComplexControl(ComplexControl cc,
  ...
  if (cc == CC_ToolButton) {
    if (const QStyleOptionToolButton *toolbutton =
      ...
      if (toolbutton->subControls & SC_ToolButtonMenu) {
        ...
        newBtn.rect = QRect(ir.center().x() + 1 - (mbi - 6) / 2,
