...
static QString FormatColor(const Colors &colors) {
  ...
  for (const Color &col : colors)
    result += QString(", stop:") + QString::number(col.coefficient) + " " +
              FormatColor(col.value);
  result += ")";
...
std::unique_ptr<StyleParams> CreateStyle(const QString &styleName) {
  ...
  if (creator == styleMap->end())
    throw std::runtime_error(styleName.toStdString() +
...
void UnregisterStyle(const QString &styleName) { styleMap->remove(styleName); }

QStringList GetRegisteredStyles() { return styleMap->keys(); }
...
#if (QT_VERSION <= QT_VERSION_CHECK(5, 10, 0))
  auto scrNumber =
      QApplication::desktop()->screenNumber(widget.mapToGlobal(QPoint(0, 0)));
