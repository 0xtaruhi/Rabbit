...
void Builder::SetCustomWidgetCreator(
    const QString &name, const std::function<QWidget *()> &creator) {
  customWidgetCreators[name] = [creator](const QJsonObject &) {
    return creator();
  };
}
...
TabToolbar *Builder::CreateTabToolbar(const QString &configPath) {
  ...
  TabToolbar *tt =
      new TabToolbar((QWidget *)parent(), groupHeight, groupRowCount);

  auto CreateCustomWidget = [this, tt](const QString &name,
                                       const QJsonObject &item) {
    if (!customWidgetCreators.contains(name))
      throw std::logic_error(std::string("Unknown item type: ") +
                             name.toStdString());
  ...
  auto CreateActionParams = [&actionsMap, &menusMap](const QJsonObject &obj) {
    ...
    if (type == "delayedPopup")
      ...
      throw std::logic_error(std::string("Unknown toolbutton popup mode: ") +
                             type.toStdString());
  ...
  for (int i = 0; i < tabs.size(); i++) {
    ...
    for (int j = 0; j < groups.size(); j++) {
      ...
      for (int k = 0; k < content.size(); k++) {
        ...
        static const QList<QString> defaultTypes = {"action", "subgroup",
                                                    "separator"};
        ...
        case 1: // subgroup
        {
          ...
          for (int w = 0; w < subGroupContent.size(); w++) {
            ...
            static const QList<QString> defaultSgTypes = {"action",
                                                          "horizontalActions"};
            ...
            case 1: // horizontalActions
            {
              ...
              for (int x = 0; x < horizActionsArray.size(); x++) {
                const QJsonObject horizAction =
                    horizActionsArray.at(x).toObject();
