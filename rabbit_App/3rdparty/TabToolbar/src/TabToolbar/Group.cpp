...
void Group::AddSeparator() { innerLayout->addWidget(CreateSeparator()); }

void Group::AddAction(QToolButton::ToolButtonPopupMode type, QAction *action,
                      QMenu *menu) {
  if (type == QToolButton::MenuButtonPopup) {
    innerLayout->addWidget(new CompactToolButton(action, menu, this));
  } else {
    const int iconSize =
        GetPixelMetric(QStyle::PM_LargeIconSize) * GetScaleFactor(*this);
