...
TabToolbar::TabToolbar(QWidget *parent, unsigned _groupMaxHeight,
                       unsigned _groupRowCount)
    : QToolBar(parent), groupRowCount(_groupRowCount),
  ...
  QObject::connect(tabBar, &QTabWidget::tabBarDoubleClicked, hideAction,
                   &QAction::trigger);
  QObject::connect(tabBar, &QTabWidget::tabBarClicked, this,
                   &TabToolbar::TabClicked);
  QObject::connect(tabBar, &QTabWidget::currentChanged, this,
                   &TabToolbar::CurrentTabChanged);
  QObject::connect((QApplication *)QApplication::instance(),
...
bool TabToolbar::event(QEvent *event) {
  if (event->type() == QEvent::StyleChange && !ignoreStyleEvent)
    QTimer::singleShot(0, this,
                       [this]() { // on KDE new palette is not ready yet, wait
                         const QString styleName =
                             (style ? style->objectName() : GetDefaultStyle());
                         SetStyle(styleName);
                       });
...
unsigned TabToolbar::RowCount() const { return groupRowCount; }
...
QAction *TabToolbar::HideAction() { return hideAction; }
...
int TabToolbar::CurrentTab() const { return currentIndex; }

void TabToolbar::SetCurrentTab(int index) { tabBar->setCurrentIndex(index); }
