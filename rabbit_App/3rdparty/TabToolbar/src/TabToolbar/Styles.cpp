...
struct DefaultStylesRegistrar {
  DefaultStylesRegistrar() {
    ...
    auto coeff = [clampColor](const QColor &c, float cr, float cg, float cb) {
      return QColor(clampColor(c.red() * cr), clampColor(c.green() * cg),
                    clampColor(c.blue() * cb));
    };
    ...
    auto toBase = [](const QColor &c) {
      ...
      return QColor(c.red() - minColor, c.green() - minColor,
                    c.blue() - minColor);
    };
    ...
    RegisterStyle(g_styleKool, [lcomb, dimmed]() {
      ...
      params->TabUnselectedHoverBorderColorSide =
          params->TabHoverBorderColorSide = {
              {0.0f, params->BorderColor},
              {0.1f, params->BorderColor},
              {0.7f, params->TabHoverBorderColorTop},
              {1.0f, params->TabHoverBorderColorTop}};

      params->PaneColor = {
          {0.0f, dimmed(p.light, 0.1f)}, {0.7f, p.light}, {1.0f, p.light}};

      params->TabSelectedColor = p.light;

      params->TabUnselectedColor =
          lcomb(params->ToolbarBackgroundColor,
                params->TabSelectedColor[0].value, 0.5f);

      params->SeparatorColor = {
          {0.0f, Qt::transparent},     {0.05f, Qt::transparent},
          {0.1f, params->BorderColor}, {0.9f, params->BorderColor},
          {0.95f, Qt::transparent},    {1.0f, Qt::transparent}};
    ...
    RegisterStyle(g_styleVienna, [lcomb, dimmed, coeff, add, toBase]() {
      const TPalette p;
      const QColor vienna = (p.window.lightnessF() > 0.5f ? QColor(51, 153, 255)
                                                          : QColor(25, 40, 70));
      StyleParams *params = new StyleParams();
      ...
      params->ToolbarBackgroundColor =
          add(dimmed(p.window, 0.07f), toBase(dimmed(vienna, 0.892f)));
      const float dimmCoeff = (p.window.lightnessF() > 0.5f ? 0.225f : 0.5f);
      params->BorderColor =
          add(dimmed(p.window, dimmCoeff), toBase(dimmed(vienna, 0.838f)));
      params->GroupNameColor =
          lcomb(p.text, params->ToolbarBackgroundColor, 0.484f);

      params->PaneColor = {
          {0.0f, params->ToolbarBackgroundColor},
          {0.5f, params->ToolbarBackgroundColor},
          {0.75f, lcomb(params->ToolbarBackgroundColor, p.light, 0.5f)},
          {1.0f, p.light}};

      params->TabSpecialColor = {
          {0.0f,
           add(dimmed(p.window, 0.7125f),
               coeff(toBase(dimmed(p.highlight, 0.294f)), 1.0f, 1.29f, 1.0f))},
          {0.6f,
           add(dimmed(p.window, 0.891f),
               coeff(toBase(dimmed(p.highlight, 0.46f)), 1.0f, 0.69f, 1.0f))},
          {0.6001f,
           add(dimmed(p.window, 0.825f),
               coeff(toBase(dimmed(p.highlight, 0.362f)), 1.0f, 0.815f, 1.0f))},
          {1.0f, add(dimmed(p.window, 0.7125f),
                     coeff(toBase(dimmed(p.highlight, 0.416f)), 1.0f, 0.924f,
                           1.0f))}};

      params->TabSpecialHoverColor = {
          {0.0f, coeff(params->TabSpecialColor[0].value, 2.17f, 1.48f, 1.197f)},
          {0.6f,
           coeff(params->TabSpecialColor[1].value, 0.653f, 1.218f, 1.286f)},
          {0.6001f,
           coeff(params->TabSpecialColor[2].value, 1.69f, 1.326f, 1.191f)},
          {1.0f,
           coeff(params->TabSpecialColor[3].value, 1.768f, 1.44f, 1.255f)}};

      params->TabSpecialHoverBorderColor = params->TabSpecialBorderColor =
          add(dimmed(p.window, 0.729f),
              coeff(toBase(dimmed(p.highlight, 0.392f)), 1.0f, 0.66f, 1.0f));
      params->TabSpecialHoverBorderColorSide =
          params->TabSpecialBorderColorSide = params->TabSpecialBorderColor;
      params->TabSpecialFontColor = p.htext;
      ...
      params->TabUnselectedColor =
          add(dimmed(p.window, 0.02f), toBase(dimmed(vienna, 0.9264f)));

      params->TabSelectedColor = p.light;

      params->SeparatorColor = {{0.0f, Qt::transparent},
                                {0.075f, Qt::transparent},
                                {0.0751f, add(dimmed(p.window, 0.3125f),
                                              toBase(dimmed(vienna, 0.789f)))},
                                {0.925f, add(dimmed(p.window, 0.3125f),
                                             toBase(dimmed(vienna, 0.789f)))},
      ...
      params->HorizontalFrameBackgroundColor = {
          {0.0f,
           add(dimmed(p.window, 0.033f),
               coeff(toBase(dimmed(vienna, 0.9362f)), 1.0f, 1.38f, 1.0f))},
          {0.6f,
           add(dimmed(p.window, 0.05f),
               coeff(toBase(dimmed(vienna, 0.8333f)), 1.0f, 0.588f, 1.0f))},
          {0.6001f, add(p.window, coeff(toBase(dimmed(vienna, 0.9166f)), 1.0f,
                                        0.82f, 1.0f))},
          {1.0f, p.light}};
    ...
    RegisterStyle(g_styleThreshold, [lcomb, dimmed, coeff]() {
      ...
      const float paneDimmCoeff =
          (p.window.lightnessF() > 0.5f ? 0.03529f : 0.1f);
      params->PaneColor = dimmed(p.light, paneDimmCoeff);
      const float borderDimmCoeff =
          (p.window.lightnessF() > 0.5f ? 0.15f : 0.3f);
      params->BorderColor =
          dimmed(params->ToolbarBackgroundColor, borderDimmCoeff);
      ...
      const float unselectedDimmCoeff =
          (p.window.lightnessF() > 0.5f ? 0.008f : 0.1f);
      params->TabUnselectedColor =
          dimmed(params->ToolbarBackgroundColor, unselectedDimmCoeff);

      params->TabSelectedColor = params->PaneColor;

      params->SeparatorColor = {
          {0.0f, Qt::transparent},       {0.05f, Qt::transparent},
          {0.051f, params->BorderColor}, {0.95f, params->BorderColor},
          {0.951f, Qt::transparent},     {1.0f, Qt::transparent}};
