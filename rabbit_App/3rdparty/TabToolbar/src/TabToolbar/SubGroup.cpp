...
SubGroup::SubGroup(Align align, QWidget *parent) : QFrame(parent) {
  ...
  QSpacerItem *spacer = new QSpacerItem(
      ...
      (align == Align::Yes ? QSizePolicy::Expanding : QSizePolicy::Ignored));
  innerLayout->addItem(spacer);
  setLayout(innerLayout);
}

void SubGroup::AddAction(QToolButton::ToolButtonPopupMode type, QAction *action,
                         QMenu *menu) {
  const int iconSize =
      GetPixelMetric(QStyle::PM_SmallIconSize) * GetScaleFactor(*this);
